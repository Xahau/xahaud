...
class HybridClockSpike_test : public beast::unit_test::suite
{
    ...
    {
        testcase(
            "clock contract: scheduler clock IS the consensus clock, but is "
            "NOT an asio clock");

        //@@start spike-clock-seam
        // SEAM (review claim #3 — the seam is real, only the spec's "up-casts"
        // prose was loose). csf::Scheduler::clock() returns
        // ManualClock<steady>&, which IS-A AbstractClock<steady>& — exactly
        // Consensus's clock_type.
        ...
        static_assert(
            std::is_convertible_v<SteadyManual&, ConsensusClock const&>);
        // ...but they are DIFFERENT typedefs: the scheduler exposes the
        // concrete ManualClock; consensus wants the abstract interface.
        static_assert(
            !std::is_same_v<csf::Scheduler::clock_type, ConsensusClock>);
        //@@end spike-clock-seam

        //@@start spike-no-static-now
        // KILL-CLAIM (review #2). asio basic_waitable_timer<Clock> needs a
        // STATIC Clock::now(). steady_clock has one; ManualClock's now() is a
        // virtual INSTANCE method (abstract_clock.h:51, manual_clock.h:36). So
        // the very per-node ManualClock that drives consensus cannot also back
        // an asio timer — "template the timer Clock to a ManualClock" is
        // impossible.
    ...
    testVirtualTimePumpsHeartbeat()
    {
        ...
        testcase(
            "virtual time deterministically pumps a self-rescheduling "
            "heartbeat (the v2 driver)");

        csf::Scheduler scheduler;

        //@@start spike-virtual-pump
        // The v2 driver mechanism the review prescribes. Advancing the clock
        // and hoping an asio timer fires is a no-op (heartbeatTimer_ is a real
        ...
        // Scheduler advances VIRTUAL time and fires the heartbeat
        // deterministically with no wall-clock sleeping (this test advances 10s
        // instantly).
        constexpr auto heartbeat =
            1s;  // stand-in for ConsensusParms::ledgerGRANULARITY
        int beats = 0;
    ...
    testTransportSubstrate()
    {
        using namespace std::chrono_literals;
        testcase(
            "BasicNetwork is a generic, partitionable transport (substrate "
            "reuse)");

        csf::Scheduler scheduler;

        //@@start spike-transport-substrate
        // BasicNetwork<Peer> is already a template over the vertex type; csf
        // only instantiates BasicNetwork<csf::Peer*> by convention. Here we
        // drive it with a NON-csf vertex (int, standing in for a RealNode id)
        // carrying an arbitrary payload — proving it can host real-Application
        // nodes unchanged. connect()/disconnect() are the partition/eclipse
        // primitives.
        ...
        int delivered = 0;
        net.send(
            1, 2, [&] { ++delivered; });  // payload would carry Message bytes
    ...
    testQuiescenceBarrierFixpoint()
    {
        using namespace std::chrono_literals;
        testcase(
            "quiescence barrier needs a bounded predicate (self-rescheduling "
            "work has no empty-queue fixpoint)");

        csf::Scheduler scheduler;

        //@@start spike-quiescence-fixpoint
        // The review's determinism caveat, demonstrated. Self-rescheduling
        // events (like the acquire retry timers, TimeoutCounter) keep the event
        // queue NON-empty forever, so a "drain until the queue is empty"
        // barrier would never return. A correct quiescence barrier must use a
        // BOUNDED predicate (a virtual deadline / progress condition) — which
        // terminates cleanly.
        int work = 0;
        ...
        BEAST_EXPECT(
            scheduler.now() == t0 + 5s);  // bounded predicate terminated
    ...
    {
        testcase(
            "real Application: post a heartbeat-style job and drain to "
            "quiescence (Stage-2 driver primitive)");
        jtx::Env env{*this};
        ...
        //   jobQueue_.addJob(jtNETOP_TIMER, "NetHeart", []{
        //   processHeartbeatTimer(); })
        // (NetworkOPs.cpp:1067). Crucially, processHeartbeatTimer RELEASES the
        // master mutex (its lock block ends at NetworkOPs.cpp:1164) BEFORE
        // calling consensus_.timerEntry() at NetworkOPs.cpp:1166 — so
        // timerEntry does NOT run under the master mutex. That corrects the
        // review's "holds master mutex across timerEntry" deadlock fear:
        // virtualizing the heartbeat only changes WHO/WHEN posts the job, while
        // the JobQueue worker runs it with prod's exact (already deadlock-safe)
        // locking. Here we post a job to the REAL JobQueue and drain it to
        // quiescence via rendezvous() — proving the post-and-drain driver
        // primitive works against a real Application.
        std::atomic<int> ran{0};
        auto& jq = env.app().getJobQueue();
        BEAST_EXPECT(jq.addJob(jtCLIENT, "spike", [&] { ran.fetch_add(1); }));
        jq.rendezvous();  // bounded drain-to-quiescence (cf.
                          // spike-quiescence-fixpoint)
