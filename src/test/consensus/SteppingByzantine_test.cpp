...
class SteppingByzantine_test : public beast::unit_test::suite
{
    ...
    sawValidationPrefix(SteppingNetwork& net, uint256 const& hash)
    {
        ...
        return std::any_of(
            ...
            [&prefix](std::string const& ev) {
                return ev.find(prefix) != std::string::npos;
            });
    }
    ...
    testEquivocatingValidator()
    {
        ...
        expectReplays(
            *this, "equivocating validator", [&](SteppingNetwork& net) {
                ...
                // sends. Peers receive both: equivocation from a trusted
                // signer.
                ...
                // twisted view its own lies produced — we ask only the honest
                // set.
                ...
                if (!BEAST_EXPECT(net.validatedAgree({0, 1, 2, 3}, target)))
                {
                    for (std::uint32_t i = 0; i < n; ++i)
                        log << "  n" << i << " valid=" << net.validSeq(i)
                            << " mode=" << static_cast<int>(net.mode(i))
                            << std::endl;
                    return Payload{};
                }
                ...
                // have REACHED an honest node's validation machinery —
                // otherwise the scenario proves nothing about byzantine
                // tolerance, it just proves a healthy network converges. Scan
                // the recorded validation lifecycle for a fabricated hash
                // prefix.
                for (std::size_t i = 0; i < lies.size(); ++i)
                    for (std::size_t j = i + 1; j < lies.size(); ++j)
                        if (!BEAST_EXPECT(
                                fakePrefix(lies[i]) != fakePrefix(lies[j])))
                            return Payload{};
    ...
    testWithholdingQuorumStarvation()
    {
        ...
        expectReplays(
            *this, "withholding quorum starvation", [&](SteppingNetwork& net) {
                using Payload = std::optional<std::vector<uint256>>;
                net.canonicalJobs();
                net.validators(n).mesh();
                if (!BEAST_EXPECT(net.allUp() && net.meshReady()))
                    ...
                    for (std::uint32_t p = 0; p < n; ++p)
                        if (p != b)
                            net.faultLink(
                                b,
                                p,
                                simfaults::dropType(protocol::mtVALIDATION));

                auto const honestValidated = [&net]() {
                    return std::min(
                        {net.validSeq(0), net.validSeq(1), net.validSeq(2)});
                };
                ...
                {
                    log << "  honest tip advanced " << stalledAt << " -> "
                        << honestValidated() << " (not starved)" << std::endl;
                    return Payload{};
                }
                ...
                log << "  starved: honest validated held at " << stalledAt
                    << ", closed advanced to " << net.closedSeq(0) << std::endl;
    ...
    testPerPeerEquivocation()
    {
        ...
        expectReplays(
            *this, "per-peer equivocation", [&](SteppingNetwork& net) {
                using Payload = std::optional<std::vector<uint256>>;
                net.canonicalJobs();
                net.recordForensics();
                net.validators(n).mesh();
                ...
                for (int round = 0; round < 3; ++round)
                {
                    ...
                    net.lieValidationTo(byz, campA, seq, hashA)
                        .lieValidationTo(byz, campB, seq, hashB);
                    net.runTo(seq);
                    if (!BEAST_EXPECT(net.validatedForkFree()))
                        return Payload{};
                    BEAST_EXPECT(net.offThreadJobs() == 0);
                ...
                auto sawOnCamp = [&](std::vector<std::uint32_t> const& camp,
                                     uint256 const& hash) {
                    auto const prefix = fakePrefix(hash);
    ...
    testEquivocationPartitionHealUnderPerNodeK()
    {
        ...
        expectReplays(
            *this,
            "equivocation + partition/heal + per-node K",
            [&](SteppingNetwork& net) {
                using Payload = std::optional<std::vector<uint256>>;
                ...
                auto const afterBeat = [&]() {
                    ++beat;
                    forkFreeDuringPressure =
                        forkFreeDuringPressure && net.validatedForkFree();
                    ...
                    if (honestTip > byzTip)
                    {
                        ...
                        maxHonestAhead =
                            std::max(maxHonestAhead, honestTip - byzTip);
                    }
                };
                ...
                auto const afterHealBeat = [&]() {
                    ++healBeat;
                    if (targetReachedBeat == 0 &&
                        net.minValidatedSeq() >= healTarget)
                ...
                auto const healStats = net.runProfiledUntil(
                    [&net, healTarget]() {
                        return net.minValidatedSeq() >= healTarget &&
                            net.ledgersAgree(healTarget);
                    },
                    options,
                    SteppingNetwork::RunBudget{
                        /*heartbeats=*/80, /*steps=*/1'000'000},
                    SteppingNetwork::Cadence{
                        /*dt=*/seconds{1}, /*skew=*/milliseconds{20}},
                    afterHealBeat);
                ...
                if (!BEAST_EXPECT(
                        targetReachedBeat != 0 &&
                        agreementBeat >= targetReachedBeat))
                    return Payload{};
                ...
                log << "  composed Axis B: pre-heal honestTip=" << honestTip
                    ...
                    << ", healClamps=" << healStats.clampHits
                    << ", healBeats=" << healStats.beats
                    << ", targetReachedBeat=" << targetReachedBeat
                    << ", agreementBeat=" << agreementBeat
