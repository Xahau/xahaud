...
class SteppingController_test : public beast::unit_test::suite
{
    ...
    testClassify()
    {
        ...
        BEAST_EXPECT(
            cls(jtEXPORT_SHARES, "UnknownShareWork").action == A::fail);
        BEAST_EXPECT(
    ...
    testExtensionWorkScheduled()
    {
        ...
        BEAST_EXPECT(hook(jtEXPORT_SHARES, "recvExportShares", [&]() {
                         order.push_back(2);
                     }) == D::claimedQueued);
        ...
        BEAST_EXPECT(
            c.jobDiagnostics().find("JtExportShares") != std::string::npos);
    ...
    testNamedJobLagAndObservation()
    {
        ...
        using D = JobQueue::JobDisposition;
        BEAST_EXPECT(hook0(jtADVANCE, "validatedLedgerWork", [&] {
                         body(3);
                     }) == D::claimedQueued);
        ...
        BEAST_EXPECT(hook1(jtADVANCE, "validatedLedgerWork", [&] {
                         body(0);
                     }) == D::claimedQueued);
        while (c.stepOne())
        {
    ...
    testLaggedPendingJobCount()
    {
        ...
        BEAST_EXPECT(
            hook1(jtADVANCE, "validatedLedgerWork", [] {}) == D::claimedQueued);
        BEAST_EXPECT(
            c.laggedPendingJobCount(
                0, jtEXPORT_SHARES, "validatedLedgerWork") == 0);
