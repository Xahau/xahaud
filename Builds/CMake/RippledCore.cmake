...
file(GLOB_RECURSE rb_headers src/ripple/beast/*.h src/ripple/beast/*.hpp)

add_library(xrpl_core ${rb_headers}) # headers added here for benefit of IDEs
if(unity)
...
target_sources(
  xrpl_core
  PRIVATE src/ripple/beast/clock/basic_seconds_clock.cpp
          ...
          # Enhanced logging - compiles to empty when BEAST_ENHANCED_LOGGING is
          # not defined
...
target_include_directories(
  xrpl_core PUBLIC $<BUILD_INTERFACE:${CMAKE_CURRENT_SOURCE_DIR}/src>
                   $<INSTALL_INTERFACE:include>)

target_compile_definitions(
  xrpl_core PUBLIC BOOST_ASIO_USE_TS_EXECUTOR_AS_DEFAULT
                   BOOST_CONTAINER_FWD_BAD_DEQUE HAS_UNCAUGHT_EXCEPTIONS=1)
target_compile_options(xrpl_core
                       PUBLIC $<$<BOOL:${is_gcc}>:-Wno-maybe-uninitialized>)
...
# Uses generator expression so it works with multi-config generators (Xcode, VS,
# Ninja Multi-Config)
target_compile_definitions(
  xrpl_core
  PUBLIC
...
#]=================================]
install(
  FILES src/ripple/basics/base64.h
...
install(FILES src/ripple/crypto/RFC1751.h src/ripple/crypto/csprng.h
              src/ripple/crypto/secure_erase.h
        DESTINATION include/ripple/crypto)
install(
  FILES src/ripple/json/JsonPropertyStream.h
...
install(FILES src/ripple/json/impl/json_assert.h
        DESTINATION include/ripple/json/impl)
install(
  FILES src/ripple/protocol/AccountID.h
...
install(FILES src/ripple/protocol/impl/STVar.h
...
#]===================================]
install(
  FILES src/ripple/beast/clock/abstract_clock.h
        src/ripple/beast/clock/basic_seconds_clock.h
        src/ripple/beast/clock/manual_clock.h
  DESTINATION include/ripple/beast/clock)
install(FILES src/ripple/beast/core/LexicalCast.h src/ripple/beast/core/List.h
              src/ripple/beast/core/SemanticVersion.h
        DESTINATION include/ripple/beast/core)
install(FILES src/ripple/beast/hash/hash_append.h src/ripple/beast/hash/uhash.h
              src/ripple/beast/hash/xxhasher.h
        DESTINATION include/ripple/beast/hash)
install(FILES src/ripple/beast/hash/impl/xxhash.h
        DESTINATION include/ripple/beast/hash/impl)
install(FILES src/ripple/beast/rfc2616.h src/ripple/beast/type_name.h
              src/ripple/beast/unit_test.h src/ripple/beast/xor_shift_engine.h
        DESTINATION include/ripple/beast)
install(
  FILES src/ripple/beast/utility/Journal.h
        src/ripple/beast/utility/PropertyStream.h
        src/ripple/beast/utility/Zero.h src/ripple/beast/utility/rngfill.h
  DESTINATION include/ripple/beast/utility)
# WARNING!! -- horrible levelization ahead (these files should be isolated or
# moved...but unfortunately unit_test.h above creates this dependency)
if(tests)
  install(
    FILES src/ripple/beast/unit_test/amount.hpp
  ...
  install(FILES src/ripple/beast/unit_test/detail/const_container.hpp
...
if(tests)
  target_sources(
    rippled
    PRIVATE src/test/app/AccountDelete_test.cpp
...
target_link_libraries(
  ...
  # external/snappy/conanfile.py for a full explanation. This is likely not
  # strictly necessary, but listed explicitly as a good practice.
  m)
exclude_if_included(rippled)
# define a macro for tests that might need to be exluded or run differently in
# CI environment
if(is_ci)
