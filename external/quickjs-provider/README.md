...
declarations, selected-surface manifest, generated XFL profile ledger, and
API-artifact manifest. CI checks out jshookz `main` and fails if
`build provider` does not reproduce that lock. The importer then prints a
self-contained `x-run-tests` command that regenerates the embedded Hook
fixtures and verifies the cut.
...
`api-artifacts.json` closes the four tracked API artifacts; Xahau verifies its
